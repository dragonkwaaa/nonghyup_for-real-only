...
<script src="/product/js/bundleInfo.js"></script>
...
$(window).scroll(function() {
	...
	// :: 애니메이션 없는 졸졸이 따라다니는 스크립트
	// $(".separatedRight.rightFloat.innerFloat.goodsInfoGroup").css('top', newPosition);
	 

	// :: 애니메이션 들어간 졸졸이 따라다니는 스크립트. 
	// :: [조건] 상품 상세내용 시작되는 탭 보다 현재 보는 화면이 아래로 내려갔을 때. 
	if(scrollTop > limitTop - 150) {
		$(".separatedRight.rightFloat.innerFloat.goodsInfoGroup").stop().animate({
			"top" : newPosition
		}, 500);
	} 
	// :: [조건] 현재 보는 화면의 밑바닥이 전체 페이지의 바텀+480px 보다 낮을 때. 
	if (scrollBottom < 480) {
		$(".separatedRight.rightFloat.innerFloat.goodsInfoGroup").stop().animate({
			"top" : limitFooter - 750
			// "bottom" : 450
