	...
	postService(url, dataType, param, function(data){
		...
		if(reviewList.length){
			...
			$.each(reviewList, function(index, re) {
				...
				// :: ksg_20200422_2224 single : 선택 가능한 리뷰 제목 작업 파트
	...
	postService(url, dataType, param, function(data){
		...
		if(askList.length){
			...
			$.each(askList, function(index, ask) {
				...
				str1 			+=		'			<span class="ml30">'+ask.userID+'</span>';
				str1 			+=		'			<span class="absoluteR">'+ask.regDate.substring(0,10)+'</span>';
				str1 			+=		'		</a>';
...
// :: ksg_20200423_1102 single : 상품 후기 슬라이드 토글 스크립트
